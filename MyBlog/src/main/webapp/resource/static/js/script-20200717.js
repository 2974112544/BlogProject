...
$(document).on('click', '#cancel-comment-reply-link', function () {
    var commentId = $(this).attr('data-id');
    var anchorCurrent = $('#anchor-' + commentId).html();
    $('#respond-title').html('发表评论');
    $('#anchor-' + commentId).remove();
    $('#anchor-top').html(anchorCurrent);
...
function runTime() {
    var d = new Date(), str = '';
    var BirthDay = new Date("2016-11-27");
    var today = new Date();
    ...
    var daysold = Math.floor(e_daysold) - yearsold * 365;
    str = yearsold + "年";
    str += daysold + "天";
    str += d.getHours() + '时';
    str += d.getMinutes() + '分';
